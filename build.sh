#!/usr/bin/env bash
# Build Verity MiMo Addon -> a single standalone jar
set -euo pipefail
cd "$(dirname "$0")"
...
CP_LIBS="build/lib/gson.jar:build/lib/cloth-config.jar:build/lib/mixin.jar:build/lib/annotations.jar:build/lib/guava.jar"
...
(cd build/out && zip -q -r -9 ../../Verity-MiMo-Addon.jar .)

echo "== done: $(pwd)/Verity-MiMo-Addon.jar =="
